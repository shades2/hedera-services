...
public final class TokenConversion {
	...
	private static void initModelFields(final Token token, final MerkleToken immutableToken) {
		...
		token.setFrozenByDefault(immutableToken.frozenByDefault());
		token.setKycGrantedByDefault(immutableToken.kycGrantedByDefault());
		token.setAdminKey(immutableToken.getAdminKey());
		token.setFeeScheduleKey(immutableToken.feeScheduleKeyIfPresent());
		token.setType(immutableToken.type());
		token.setLastUsedSerialNumber(immutableToken.lastUsedSerialNumber());
		token.setDeleted(immutableToken.isDeleted());
