...
public final class PresolvencyFlaws {
	static final Map<ResponseCodeEnum, Pair<TxnValidityAndFeeReq, SwirldTxnAccessor>> WELL_KNOWN_FLAWS =
	...
	static Pair<TxnValidityAndFeeReq, SwirldTxnAccessor> responseForFlawed(final ResponseCodeEnum status) {
		final var response = WELL_KNOWN_FLAWS.get(status);
		return (null != response) ? response : failureWithUnknownFeeReq(status);
	}

	private static Pair<TxnValidityAndFeeReq, SwirldTxnAccessor> failureWithUnknownFeeReq(final ResponseCodeEnum error) {
		return Pair.of(new TxnValidityAndFeeReq(error), null);
	}

	private static void putTo(
			final Map<ResponseCodeEnum, Pair<TxnValidityAndFeeReq, SwirldTxnAccessor>> map,
