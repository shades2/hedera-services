...
public class ContractWrappedCallTransitionLogic implements PreFetchableTransition {
	private static final Logger log = LogManager.getLogger(ContractWrappedCallTransitionLogic.class);
	...
	@Inject
	public ContractWrappedCallTransitionLogic(
	...
	public void doStateTransition() {
		...
		var op = contractCallTxn.getContractWrappedCall();
		...
		int startIndex = op.getFunctionParameterStart();
		final var callData = startIndex > 1
				? Bytes.wrap(op.getForeignTransactionBytes()
				    .substring(startIndex, startIndex + op.getFunctionParameterLength()).toByteArray())
				: Bytes.EMPTY;
	...
	public Predicate<TransactionBody> applicability() {
		return TransactionBody::hasContractWrappedCall;
	}
	...
	private ResponseCodeEnum validateSemantics(final TransactionBody transactionBody) {
		...
		var ethTx = op.getForeignTransactionBytes();

		if (ethTx.size() < 1) {
			return FOREIGN_TRANSACTION_FORMAT_ERROR;
		}

		return switch (ethTx.byteAt(0)) {
			case 1 -> validateEIP2930TxSemantics(transactionBody);
			case 2 -> validateEIP1559TxSemantics(transactionBody);
			default -> validateFrontierTxSemantics(transactionBody);
		};
	...
	private ResponseCodeEnum validateFrontierTxSemantics(final TransactionBody transactionBody) {
		var op = transactionBody.getContractWrappedCall();
		try {
			var input = RLP.input(Bytes.wrap(op.getForeignTransactionBytes().toByteArray()));
			if (input.enterList() != 9) {
				return FOREIGN_TRANSACTION_FORMAT_ERROR;
			}
	...
	private ResponseCodeEnum validateEIP1559TxSemantics(final TransactionBody transactionBody) {
		var op = transactionBody.getContractWrappedCall();
		if (op.getForeignTransactionBytes().byteAt(0) != 0x02) {
			return FOREIGN_TRANSACTION_FORMAT_ERROR;
		}
		try {
			var input = RLP.input(Bytes.wrap(op.getForeignTransactionBytes().toByteArray()).slice(1));
			if (input.enterList() != 12) {
				return FOREIGN_TRANSACTION_FORMAT_ERROR;
			}
			if (input.readIntScalar() != properties.getChainId()) {
				return FOREIGN_TRANSACTION_INCORRECT_CHAIN_ID;
			...
			var priorityFee = input.readLongScalar();
			var maxGasFee = input.readLongScalar();
			var gasLimit = input.readLongScalar();
			if (gasLimit != op.getGas()) {
				return FOREIGN_TRANSACTION_INCORRECT_GAS_LIMIT;
			}
