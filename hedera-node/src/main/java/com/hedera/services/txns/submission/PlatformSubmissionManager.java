...
public class PlatformSubmissionManager {
	...
	public ResponseCodeEnum trySubmission(SwirldTxnAccessor accessor) {
	...
	private SwirldTxnAccessor effective(SwirldTxnAccessor accessor) {
		var txn = accessor.getTxn();
		if (txn.hasUncheckedSubmit()) {
			try {
				return new SwirldTxnAccessor(txn.getUncheckedSubmit().getTransactionBytes().toByteArray());
