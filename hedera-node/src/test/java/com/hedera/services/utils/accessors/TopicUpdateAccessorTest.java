...
class TopicUpdateAccessorTest {
	...
	private SwirldTransaction topicUpdateTxn;

	@Test
	void getsMetaCorrectly() throws InvalidProtocolBufferException {
		final TransactionBody updateTxn = TransactionBody.newBuilder()
				...
				.build();
		givenTxn(updateTxn);
		accessor = new TopicUpdateAccessor(topicUpdateTxn, aliasManager);
	...
	void givenTxn(TransactionBody body) {
		topicUpdateTxn = new SwirldTransaction(Transaction.newBuilder()
